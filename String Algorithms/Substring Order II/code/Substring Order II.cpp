...
void DP_path() {
    for(int i = 0; i < sz; ++i) {
        ...
        for(map<char, int>::iterator it = SA[u].Next.begin(); it != SA[u].Next.end(); ++it)
            SA[u].dp = Add(SA[u].dp, SA[it->second].dp);
    }
}
...
int main() {
    ...
    Topo_Sort();
    for(int i = 0; i < sz; ++i)
    if (SA[Topo[i]].link >= 0) SA[SA[Topo[i]].link].cnt += SA[Topo[i]].cnt;
    SA[0].cnt = 0;
    for(int i = sz-1; i >= 0; --i) SA[Topo[i]].dp = SA[Topo[i]].cnt;
    DP_path();
